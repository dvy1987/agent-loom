...
Block-level protection during **`code-simplification`** (adapted from addyosmani/agent-skills). Mark code that should never be simplified — the model won't see it.
...
3. Invoke **`code-simplification`** — protected blocks become `/* BLOCK_de115a1d: perf-critical */` placeholders. The model reasons about surrounding code without seeing the protected implementation.
