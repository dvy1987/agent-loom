...
Cross-session citation cache for [`source-driven-development`](../.agents/skills/source-driven-development/SKILL.md), [`research-skill`](../.agents/skills/research-skill/SKILL.md), and the `learn-from-*` family. Skips redundant `WebFetch` calls without weakening the skill's "verify against current docs" guarantee.
