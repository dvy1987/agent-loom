...
| [`api-deprecation-and-migration`](.agents/skills/api-deprecation-and-migration/) | Retire application APIs and features safely — graduated deprecation, migration guides, zero-usage removal | Deprecation notice + migration progress in chat | "API deprecation", "migrate users off", "sunset this endpoint", "strangler migration" |
...
| [`browser-testing-with-devtools`](.agents/skills/browser-testing-with-devtools/) | Verify UI in a real browser via Chrome DevTools MCP — DOM, console, network, performance, screenshots | Browser verification report in chat | "browser testing", "test in Chrome", "DevTools MCP", "check console errors" |
...
| [`performance-optimization`](.agents/skills/performance-optimization/) | Measure-first profiling — baseline, find bottlenecks, fix, verify, guard against regression | Before/after metrics + guard plan in chat | "performance optimization", "profile this", "slow page", "fix LCP", "reduce bundle size" |
| [`shipping-and-launch`](.agents/skills/shipping-and-launch/) | Safe production launches — pre-launch checklist, staged rollout, monitoring, rollback plan | Shipping plan + post-deploy checks in chat | "shipping and launch", "pre-launch checklist", "production deploy", "rollback strategy" |
