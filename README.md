...
New here? Get a **real verified edit** on an offline fixture — no API keys:

```text
Run the agent-loom quickstart
```

Or browse the product tour: [`examples/`](examples/) (safe-change, structured-planning, deploy-anywhere demos). See [`docs/why-agent-loom.md`](docs/why-agent-loom.md).
...
| [`code-simplification`](.agents/skills/code-simplification/) | Simplify application code for clarity without changing behavior — incremental refactors after tests pass | Refactor commits advised; behavior preserved | "simplify this code", "code simplification", "reduce complexity", "refactor for clarity" |
| [`api-and-interface-design`](.agents/skills/api-and-interface-design/) | Contract-first APIs and module boundaries — consistent errors, boundary validation, additive changes | API contract + schema summary in chat | "API design", "design the API", "module boundary", "API contract" |
| [`context-engineering`](.agents/skills/context-engineering/) | Build minimal, high-signal context for AI coding tasks — constraints, repo facts, key files, and verification plan | Copy/pasteable context bundle + next action in chat | "context engineering", "gather context", "what do you need from me", "before you start" |
| [`app-security-hardening`](.agents/skills/app-security-hardening/) | Harden application code against common risks — boundary validation, least privilege, safe secrets and logging, dependency hygiene | Hardening checklist + verification outcome in chat | "security hardening", "OWASP hardening", "secure this feature", "harden this" |
| [`ci-cd-and-automation`](.agents/skills/ci-cd-and-automation/) | Design CI/CD pipelines and automation — fast PR gates, safe releases, deterministic workflows | CI/CD plan + workflow outline + verification steps in chat | "add CI", "CI/CD", "GitHub Actions", "set up workflows" |
| [`performance-optimization`](.agents/skills/performance-optimization/) | Measure, profile, and fix performance bottlenecks — Core Web Vitals, latency, bundle size, query tuning | Performance report with baseline, fix, delta, and guard in chat | "optimize performance", "Core Web Vitals", "LCP", "profile bottleneck" |
| [`shipping-and-launch`](.agents/skills/shipping-and-launch/) | Safe production launches — checklist, monitoring, staged rollout, rollback | Launch plan + checklist + rollout stages in chat | "launch checklist", "go live", "rollout plan", "rollback plan" |
| [`browser-testing-with-devtools`](.agents/skills/browser-testing-with-devtools/) | Test and debug browser UIs via Chrome DevTools MCP — DOM, console, network, traces | Browser verification report with evidence in chat | "browser testing", "Chrome DevTools", "verify UI fix" |
| [`api-deprecation-and-migration`](.agents/skills/api-deprecation-and-migration/) | Deprecate APIs and migrate consumers safely — timelines, tooling, sunset | Deprecation plan + migration guide outline in chat | "deprecate API", "API migration", "sunset endpoint" |
...
| [`architectural-decision-log`](.agents/skills/architectural-decision-log/) | Capture the "why" behind technical choices to prevent architectural drift. Two modes: `INTERACTIVE` (default — contemporaneous) and `SYNTHESIS=true` (retrospective backfill from repo state, used by `retroactive-project-setup`). | **File created:** `docs/adr/` + logged | "record a decision", "write an ADR", "why did we do this", `architectural-decision-log SYNTHESIS=true` |
...
| [`retroactive-project-setup`](.agents/skills/retroactive-project-setup/) | Bootstrap full agent infrastructure (AGENTS.md, architecture, soul, ADR-0001, memory seed) over an existing codebase without modifying source code — surveys repo, infers from manifests/README/git, asks only about genuine gaps | **Files created:** `AGENTS.md`, `docs/architecture.md`, `docs/product-soul.md`, `docs/adr/ADR-0001-*.md`, `docs/memory/*` + logged | "retroactive project setup", "backfill agent infrastructure", "bootstrap agents for this existing repo", "set up agents without touching code" |
| [`agent-loom-sync`](.agents/skills/agent-loom-sync/) | Sync library skills from an agent-loom upstream repo (`../agent-loom`) into project `.agents/skills/` via rsync — preserves project-local and forked skills; dry-run plan before apply | **Config:** `.agents/agent-loom-sync.json` + updated skill dirs + logged | "sync agent-loom", "update skills from upstream", "rsync from ../agent-loom", "pull skill library updates" |
...
| Skill | What it does | Output / Outcome | Trigger phrases |
|-------|-------------|-----------------|----------------|
| [`frontend-design`](.agents/skills/frontend-design/) | Orchestrator + builder — derives the stack, routes direction → system → build → review, builds from golden examples with mandatory polish + state-coverage gates and anti-vibecoded hard bans (no Inter-on-purple, no Lucide-everywhere, no centered-hero+2-CTA) | **Files created:** `.design/<feature>/DIRECTION.md`, `REVIEW.md`, `DESIGN.md`, `src/styles/tokens.css` + build + logged | "build a frontend", "design a UI", "make this not look AI-generated" |
| [`design-direction`](.agents/skills/design-direction/) | The anti-generic gate — sets a deliberate posture then explores **2-3 genuinely distinct directions** (not three palettes of one idea) and commits to one; absorbs the 12-archetype catalog as a starting palette | **File created:** `.design/<feature>/DIRECTION.md` + logged | "pick a design direction", "explore some looks", "make this feel like Linear" |
...
| [`validate-skills`](.agents/skills/validate-skills/) | Read-only health check — scores every skill, flags failures, size violations, duplicate triggers, and producer-skill memory-checkpoint gaps | **No files modified.** Structured quality report with P0/P1/P2/P3 actions | Pre-flight for `improve-skills`; quality gate after creation |
