...
Generated by `library-skill` on 2026-06-29 (Phase 2 batch 3 + post-improve-skills hygiene).

Visual map of how skills in agent-loom call each other. Arrows show direction of invocation (`caller → callee`). Skills with no outgoing arrows are leaf nodes. 102 skills across meta, thinking, and project-specific categories (`domain` reserved, currently empty), plus the `setup-evaluator` agent.
