...
Generated by `library-skill` on 2026-06-01 (partial update: Phase 2 batch 1 skills).

Visual map of how skills in agent-loom call each other. Arrows show direction of invocation (`caller → callee`). Skills with no outgoing arrows are leaf nodes. 95 skills across meta, thinking, and project-specific categories (`domain` reserved, currently empty), plus the `setup-evaluator` agent.
