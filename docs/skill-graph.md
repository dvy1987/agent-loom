...
Generated by `library-skill` on 2026-07-04 (Phase 2 gap skills + L3 enrichment pass); incrementally updated 2026-07-08 (agentic quality loop skills added).

Visual map of how skills in agent-loom call each other. Arrows show direction of invocation (`caller → callee`). Skills with no outgoing arrows are leaf nodes. 102 skills across meta, thinking, and project-specific categories (`domain` reserved, currently empty), plus the `setup-evaluator` agent, **as of the 2026-07-04 full regeneration** — this file has not been fully regenerated since, so several skill families added after that date (harness engineering, safe-change/planning/observability/deploy, svg/gsap/motion animation) are not yet drawn as nodes. A full `library-skill` Step 5 regeneration is recommended before relying on this file as an exhaustive map; the 2026-07-08 addition below is an incremental patch only.
...
flowchart LR
  ...
  subgraph PS_Engineering["PS: Engineering"]
    ...
    sddv[source-driven-development]
    cs[code-simplification]
    aid[api-and-interface-design]
    ce[context-engineering]
    ash[app-security-hardening]
    cicd[ci-cd-and-automation]
    perf[performance-optimization]
    ship[shipping-and-launch]
    btd[browser-testing-with-devtools]
    adm[api-deprecation-and-migration]
    inc[incremental-implementation]
  ...
  subgraph PS_Quality["PS: Agentic Quality Loop (shipped products)"]
    ao[agent-observability]
    arr[agent-run-retro]
    rll[runtime-learning-loop]
  end
  ...
  %% --- Agentic quality loop (shipped products) ---
  asa --> ao
  se --> ao
  arr --> ao
  arr --> ep
