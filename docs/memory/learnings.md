...
## 2026-06-30 — Generic AI UI is a process problem: explore distinct directions, consolidate intent, show positive examples

- **What:** The original frontend-design suite (5 skills) produced generic, unpolished output despite extensive ban lists. Root causes: (1) `design-archetype` picked exactly ONE archetype instantly — models converge on the corpus mean without comparing options; (2) taste encoded only as "Nevers" with no golden example code; (3) design intent scattered across ARCHETYPE/TOKENS/ICONS files; (4) build step under-specified polish and state coverage.
- **Fix:** Rebuilt to 4 skills. `design-direction` explores 2-3 genuinely distinct directions before committing. `design-system` emits ONE canonical `DESIGN.md` with state-level tokens (8-step ramp, every interactive state, APCA). `frontend-design` builds from golden examples with mandatory empty/loading/error states. `design-review` adds APCA + state-coverage gates. Salvaged all taste content before deprecating old skills.
- **Why it matters:** Bans alone cannot escape the corpus mean — models need positive examples and a forced exploration step. Single-source-of-truth `DESIGN.md` prevents per-screen drift. This pattern applies to any taste-heavy skill suite where output quality depends on aesthetic judgment.
- **Revisit when:** User reports design skills still producing generic output — check whether `design-direction` exploration was skipped or golden examples were not read during build.
