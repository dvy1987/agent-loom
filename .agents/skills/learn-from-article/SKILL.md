...
Match insights to existing skills and apply per `learn-from` shared application protocol, including the mandatory **Post-Application Hardening Cycle** on every modified/created skill: modified-skill security sweep via ALL `secure-*` skills, 200-line gate via `compress-skill` / `split-skill`, then `validate-skills` (â‰¥10/14).
